...
def create_trajectory_datasets(
    ...
    for i in range(num_trajectories):
        ...
        y0 = jax.random.uniform(y_key, shape=(q_dim*2,))*2-1
        radius = jax.random.uniform(r_key)*1.8 + 0.2
        y0 = y0 / jnp.linalg.norm(y0) * radius 
       
        noise = jax.random.normal(noise_key, shape=y0.shape) * noise_std
        y0_noisy = y0 + noise
        q0 = y0_noisy[:q_dim]
        v0 = y0_noisy[q_dim:]
        initial_lag_state = (0.0, q0, v0)
        initial_energies_list.append(radius)
        
        t_traj, q_traj, v_traj = solver_true(initial_lag_state, t_eval)
